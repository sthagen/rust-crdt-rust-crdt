...
/// A tree identifier uniquely locates an element in an LSeq tree.
/// It represents the path that needs to be taken in order to reach
/// the element. At each level we store the index of the child tree node
...
pub struct IdentGen<A: Actor> {
    ...
    boundary: u64,
    /// We keep a cache of the strategy chosen for each level of the tree
    #[serde(skip)] // TODO: implement serialize/deserialize
    strategy_vec: BitVec,
    /// Site id of the trees generated by this generator
    pub site_id: A,
}
...
impl<A: Actor> IdentGen<A> {
    ...
    pub fn new_with_args(site_id: A, base: u8, boundary: u64) -> Self {
        IdentGen {
            ...
            match (p.path.get(depth), q.path.get(depth)) {
                ...
                (Some(a), Some(b)) => {
                    if a.0 == b.0 {
                        ...
                        // identifier
                        let next_index = self.index_in_range(a.0 + 1, b.0, depth);
                ...
                (None, None) => {
                    let next_index = self.index_in_range(1, self.arity_at(depth), depth);
    ...
    fn alloc_from_lower(&mut self, p: &Identifier<A>, mut depth: usize) -> Identifier<A> {
        loop {
            match p.path.get(depth) {
                Some((ix, _)) => {
                    if ix + 1 < self.arity_at(depth) {
                        let next_index = self.index_in_range(ix + 1, self.arity_at(depth), depth);
                        return self.replace_last(p, depth, next_index);
                    } else {
    ...
    fn alloc_from_upper(
        ...
        let upper = match q.path.get(depth) {
            Some(b) => b.0,
            None => self.arity_at(depth),
        };

        let next_index = self.index_in_range(1, upper, depth);
    ...
    fn arity_at(&self, depth: usize) -> u64 {
        let base_bits = (self.initial_base_bits as u32) + (depth as u32);
        assert!(base_bits < 31, "maximum depth exceeded");
    ...
    fn index_in_range(&mut self, lower: u64, upper: u64, depth: usize) -> u64 {
        assert!(
            lower < upper,
        ...
        let interval = std::cmp::min(self.boundary, upper - lower);
    ...
    fn strategy(&mut self, depth: usize) -> bool {
        match self.strategy_vec.get(depth) {
            None => {
                let new_strategy = rand::thread_rng().gen();
                self.strategy_vec.push(new_strategy);
                new_strategy
            }
            Some(s) => *s,
        }
...
impl<A: quickcheck::Arbitrary + Actor> quickcheck::Arbitrary for Identifier<A> {
    fn arbitrary<G: quickcheck::Gen>(g: &mut G) -> Identifier<A> {
        ...
        let min_depth = 1;
        let chosen_depth = u8::arbitrary(g) % (max_depth - min_depth) + min_depth;
        let mut path = Vec::new();
        for depth in 0..chosen_depth {
            let i = u64::arbitrary(g) % (2u64.pow((DEFAULT_INITIAL_BASE + depth).into()) + 1);
        ...
        if path.last().unwrap().0 == 0 {
            path.pop();
            let i = 1 + u64::arbitrary(g) % 2u64.pow((DEFAULT_INITIAL_BASE + chosen_depth).into());
            assert_ne!(i, 0);
            path.push((i, Option::arbitrary(g)));
        };
