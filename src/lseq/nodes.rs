...
pub struct AtomPayload<V: Ord + Clone, A: Actor> {
    pub clock: VClock<A>,
    pub value: Option<V>,
}

impl<V: Ord + Clone + Display, A: Actor + Display> Display for AtomPayload<V, A> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match &self.value {
...
pub enum Atom<V: Ord + Clone, A: Actor> {
    Node {
        payload: AtomPayload<V, A>,
        children: SiblingsNodes<V, A>,
    },
    MajorNode {
        payload: MiniNodes<V, A>,
        children: SiblingsNodes<V, A>,
    },
}
...
impl<V: Ord + Clone + Display, A: Actor + Display> Display for Atom<V, A> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match &self {
            Atom::Node { payload, .. } => write!(f, "Node('{}')", payload),
            Atom::MajorNode { payload, .. } => {
                let mut output = String::from("MiniNodes[ ");
                payload.iter().for_each(|(_, v)| {
